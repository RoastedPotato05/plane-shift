...
namespace EzySlice {
    ...
    public sealed class Slicer {
        ...
        private static SlicedHull CreateFrom(SlicedSubmesh[] meshes, List<Triangle> cross, int crossSectionIndex) {
            ...
            Mesh crossHull = CreateCrossMesh(cross);

            return new SlicedHull(upperHull, lowerHull, crossHull);
        }
        ...
        private static Mesh CreateCrossMesh(List<Triangle> crossSection) {
            if (crossSection == null || crossSection.Count == 0) {
                return null;
            }

            int totalTriangles = crossSection.Count;
            ...
            for (int i = 0; i < totalTriangles; i++) {
                Triangle tri = crossSection[i];
                vertices[idx] = tri.positionA;
                vertices[idx + 1] = tri.positionB;
                vertices[idx + 2] = tri.positionC;
                ...
                if (tri.hasNormal) {
                    hasNormal = true;
                    normals[idx] = tri.normalA;
                    normals[idx + 1] = tri.normalB;
                    normals[idx + 2] = tri.normalC;
                }

                if (tri.hasTangent) {
                    hasTangent = true;
                    tangents[idx] = tri.tangentA;
                    tangents[idx + 1] = tri.tangentB;
                    tangents[idx + 2] = tri.tangentC;
                }
            ...
            if (hasNormal) mesh.normals = normals;
            if (hasTangent) mesh.tangents = tangents;
