...
namespace EzySlice {
    ...
    public sealed class SlicedHull {
        ...
        private Mesh cross_hull;

        public SlicedHull(Mesh upperHull, Mesh lowerHull) {
            this.upper_hull = upperHull;
            this.lower_hull = lowerHull;
            this.cross_hull = null;
        }

        public SlicedHull(Mesh upperHull, Mesh lowerHull, Mesh crossHull) {
            this.upper_hull = upperHull;
            this.lower_hull = lowerHull;
            this.cross_hull = crossHull;
        }
        ...
        public Mesh crossHull {
            get { return this.cross_hull; }
        }

        public GameObject CreateCrossSection(GameObject original, Material crossSectionMat = null) {
            if (cross_hull == null) {
                return null;
            }

            GameObject crossObject = CreateEmptyObject("CrossSection_Hull", cross_hull);

            if (crossObject != null) {
                crossObject.transform.localPosition = original.transform.localPosition;
                crossObject.transform.localRotation = original.transform.localRotation;
                crossObject.transform.localScale = original.transform.localScale;

                MeshRenderer renderer = crossObject.GetComponent<MeshRenderer>();

                if (crossSectionMat != null) {
                    renderer.sharedMaterial = crossSectionMat;
                } else {
                    var parentRenderer = original.GetComponent<Renderer>();
                    if (parentRenderer != null && parentRenderer.sharedMaterials.Length > 0) {
                        renderer.sharedMaterial = parentRenderer.sharedMaterials[0];
                    }
                }
            }
